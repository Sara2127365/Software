...
const RestaurantDetails = () => {
    ...
    useEffect(() => {
                ...
                if (docSnap.exists()) {
                    ...
                    try {
                        logoURL = await getDownloadURL(
                            ref(storage, `services/logos/${id}`)
                        );
                    } catch (error) {
                        console.warn(`No logo found for ${id}`, error);
                    }

                    try {
                        coverURL = await getDownloadURL(
                            ref(storage, `services/covers/${id}`)
                        );
                    } catch (error) {
                        console.warn(`No cover found for ${id}`, error);
                    }

                    // Add URLs to the restaurant data
                    const fullData = {
                        ...restaurantData,
                        logoURL,
                        coverURL
        ...
        const fetchProducts = async () => {
            try {
                ...
                console.error(
                    'Error fetching products from food collection:',
                    error
                );
            }
        };
    ...
    const handleAddReview = async () => {
        ...
        try {
            ...
            const updatedReviews = await getDocs(
                query(
                    collection(db, 'reviews'),
                    where('restaurantId', '==', id)
                )
            ...
            setReviews(
                updatedReviews.docs.map(doc => ({ id: doc.id, ...doc.data() }))
            );
        } catch (error) {
    ...
    useEffect(() => {
        const total = Object.values(productQuantities).reduce(
            (sum, qty) => sum + qty,
            0
        );
        setTotalItems(total);
                    ...
                    <TouchableOpacity
                        ...
                        style={[
                            ...
                            selectedCategories === cat &&
                                styles.selectedCategoriesButton
                        ]}
                    >
                                ...
                                <TouchableOpacity
                                    ...
                                    onPress={() => {
                                        ...
                                        const cartData = products.map(prod => ({
                                            ...prod,
                                            quantity:
                                                productQuantities[prod.id] || 0 // إضافة الكمية الحالية للمنتج
                                        }));
